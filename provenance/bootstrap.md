...
| 2026-09-15 | **first hand edit to the LIVING kernel** (`src/`; `seed/` remains pristine machine-regrown). The authoring port's check failed on `cost()["seconds"]` and traced it to a real loss: v1 totalled wall-clock, but the kernel's own check never pinned it — the authoring check did, one layer up — so the blind rebuild legitimately dropped it, killing the cost envelope's last-resort unit. Restored via a `COST_UNITS` set distinct from the producer-reportable `COST_KEYS` (wall-clock is the kernel's measurement, never a self-report). **Root unchanged**: parity still earns `d64cc301…`, so the claim's equivalence class absorbed a real behavioral change. The general lesson is now in `spec/verification.md` |
