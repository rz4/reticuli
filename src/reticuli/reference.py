...
which was extracted from v1 — recorded in research/provenance/bootstrap.md.
...
def _safe(root: str, name: str) -> str:
    ...
    absolute, empty, or containing a `..` component — and any path with a
    symlink component, even one whose target stays inside the claim: an
    internal link can alias generated bytes as a pinned name
    (spec/identity.md, file rules)."""
    ...
    for part in name.split("/"):
        if part in ("", "."):
            continue
        if part == "..":
            raise ClaimError(f"unsafe recipe path ('..' component): {name!r}")
        walked = os.path.join(walked, part)
        if os.path.islink(walked):
            raise ClaimError(f"unsafe recipe path (symlink component): {name!r}")
