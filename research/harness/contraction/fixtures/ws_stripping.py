"""From scratch, strict except that it strips ASCII whitespace before decoding
(so wrapped/indented base64 is accepted, junk is still rejected)."""
...
def decode(s: str) -> bytes:
    s = "".join(s.split())  # drop all ASCII whitespace, then be strict
    if len(s) % 4 != 0:
        raise ValueError("length must be a multiple of four")
    out = bytearray()
