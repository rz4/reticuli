...
The claim has since been revised again — **v2.2, `e650b524…`** — pinning
what a day of building on it exposed: no symlink or `..` component in a
declared path (an internal link can alias generated bytes as pinned ones,
and the two identity implementations disagreed about exactly that), records
([`spec/record.md`](spec/record.md)) as the crosscheck's second transport,
and the complete audit report on a self-referential claim that the v2.1
dissent identified as finding 12. **Its proof was re-earned the next day**: a
blind `gpt-5` rebuild landed on the same root with distinct bytes — the first
M3 in this lineage to carry a full cost ledger (4.6M tokens, $25.74) — and
the crosscheck was judged satisfied by the living kernel and by the regrown
kernel itself. That second judgment needed an accommodation that is itself
the next finding: the regrown kernel reads only the legacy recipe name,
because no fixture in the suite ever pinned the new one (finding 13)
